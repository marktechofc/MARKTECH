...
class Dashboard3DController {
    ...
    setupProgressAnimation() {
        const progressFill = document.querySelector('.dashboard-frame .progress-fill');
    ...
    setupDashboardActions() {
        ...
        const dashboardButtons = dashboardFrame.querySelectorAll('.dash-btn, .mock-btn');
        dashboardButtons.forEach((button) => {
            button.addEventListener('click', (event) => {
                event.stopPropagation();
                if (typeof window.selecionarPacote === 'function') {
                    window.selecionarPacote('pedido-basico.html');
                } else {
                    localStorage.setItem('selectedPackage', 'pedido-basico.html');
